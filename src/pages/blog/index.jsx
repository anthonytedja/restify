...
const BlogPage = () => {
  ...
  const deletevisibility = !sessionStorage.getItem("token")
    ? "hidden"
    : sessionStorage.getItem("restaurant_id") !== restaurantID
    ? "hidden"
    : "visible";

  useEffect(() => {
    fetch(
      `https://anthonytedja.pythonanywhere.com/restaurants/${restaurantID}/name/`
    )
      .then((res) => {
        if (res.status >= 400) {
    ...
    fetch(
      `https://anthonytedja.pythonanywhere.com/restaurants/${restaurantID}/blog/${blogID}`
    )
      .then((res) => {
        if (res.status >= 400) {
          throw Error(res.status);
    ...
    fetch(
      `https://anthonytedja.pythonanywhere.com/restaurants/${restaurantID}/blog/${blogID}/likes/`,
      {
        headers: {
  ...
  const deleteblog = (e) => {
    ...
    fetch(
      `https://anthonytedja.pythonanywhere.com/restaurants/${restaurantID}/blog/${blogID}/delete/`,
      {
        method: "DELETE",
        headers: {
          Authorization: sessionStorage.getItem("token"),
            ...
            <section className="form-section" style={{ padding: 20 }}>
              ...
              >
                ...
                <button
                  ...
                  type="submit"
                  style={{ backgroundColor: likeBtnColor }}
                  onClick={handleLike}
                >
                  {likes}&nbsp;
                  <i className={likeBtnIcon} />
                </button>
                <button
                  className="btn btn-primary"
                  type="submit"

...
const RestaurantImages = () => {
  ...
  useEffect(() => {
    fetch(
      `https://anthonytedja.pythonanywhere.com/restaurants/${restaurantID}/picture/all/`
    )
      .then((res) => {
        if (res.status >= 400) {
  ...
  const handleDelete = (event) => {
    ...
    fetch(
      `https://anthonytedja.pythonanywhere.com/restaurants/${restaurantID}/picture/${pictureID}/delete/`,
      {
        method: "DELETE",
        headers: {
      ...
      .then((res) => {
        ...
        let newResult = result.results.filter(
          (picture) => String(picture.id) !== String(pictureID)
        );
  ...
  return (
    <>
      <ul
        className="restaurant-image-container mx-auto"
        style={{ maxWidth: 1100 }}
      >

...
const FormImage = () => {
    ...
    const handleSubmit = (event) => {
        ...
        fetch(`https://anthonytedja.pythonanywhere.com/restaurants/${restaurantID}/picture/add/`, {
            method: "POST",
            headers: {
        ...
        <section className="form-section" style={{ padding: '20px 0px' }}>
            <div className="form-container">
                <form className="needs-validation" method="post" noValidate style={{ borderRadius: 10 }} onSubmit={handleSubmit}>
                    ...
                    <div className="mb-3"><label className="form-label" htmlFor="profilepicture">Image</label><input className="form-control" type="file" id="i-image" name="image" accept="image/*" required /><div className="invalid-feedback">Please provide an image.</div></div>
                    <p className="error-msg" style={{ display: errorStatus }}>{errorMsg}</p>
                    <div className="mb-3"><button className="btn btn-primary d-block w-100" type="submit" style={{ background: 'lightblue' }} value="signin">Add Image</button></div>
                </form>
            </div>
        </section>
    );
